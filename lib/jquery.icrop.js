...
Icrop.prototype.init = function() {
    ...
    this._render(function() {
        if (o.geometry && o.geometry.left && o.geometry.top && o.geometry.width) {
            ...
            self._current = self._fillGeometry();
        }

        self._zoom(self._current.width / self._frame.width, false);
        self._draggable();
        self.element.trigger('create', o.geometry);
    });
...
Icrop.prototype._zoom = function(zoom, force) {
    ...
    this.elements.img.css('width', this._current.width);
...
// Calculate geometry for the image, which will always fill the frame.
Icrop.prototype._fillGeometry = function() {
    var data = {};

    data.width = this._frame.width;
    data.height = Math.round(data.width / this._original.ratio);

    if (data.height < this._frame.height) {
        data.width = Math.round(this._frame.height * this._original.ratio);
        data.height = Math.round(data.width / this._original.ratio);

    }
    data.left = Math.round((this._frame.width - data.width) / 2);
    data.top = Math.round((this._frame.height - data.height) / 2);
